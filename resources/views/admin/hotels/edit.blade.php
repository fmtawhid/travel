...
@section('content')
    ...
    <div class="sb2-2">
        <div class="sb2-2-2">
            <ul>
                ...
                <li class="active-bre">
                    <a href="#">{{ isset($hotel) ? 'Edit Hotel' : 'Add Hotel' }}</a>
                </li>
            </ul>
        </div>

        <div class="sb2-2-add-blog sb2-2-1">
            <h2>{{ isset($hotel) ? 'Edit Hotel Details' : 'Add Hotel Details' }}</h2>
                                        ...
                                        <div class="input-field col s12">
                                            <select multiple name="amenities[]">
                                                ...
                                                @foreach($amenities as $amenity)
                                                    <option value="{{ $amenity->id }}" {{ in_array($amenity->id, old('amenities', isset($hotel) && is_array($hotel->amenities) ? $hotel->amenities : [])) ? 'selected' : '' }}>
                                                        {{ $amenity->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="row">
                                    <div class="input-field col s12">
                                        ...
                                        <div id="image-preview" style="margin-top: 10px;">
                                            <img id="hotel-img-tag" src="{{ isset($hotel) && $hotel->image ? asset('uploads/hotels/' . $hotel->image) : '' }}"
                                                style="max-width: 200px; {{ isset($hotel) && $hotel->image ? 'display: block' : 'display: none' }}; border: 1px solid #ccc; padding: 3px; border-radius: 3px;">
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="col s12">
                                        <table class="table" id="room-types-table">
                                            ...
                                            <tbody id="room-types-body">
                                                @if(isset($hotel) && $hotel->roomTypes && count($hotel->roomTypes) > 0)
                                                    @foreach($hotel->roomTypes as $index => $roomType)
                                                        <tr class="room-type-row">
                                                            <td><input type="text" name="room_types[{{ $index }}][room_type]"
                                                                    placeholder="e.g., Deluxe" style="width: 100%; padding: 5px;" value="{{ $roomType->room_type ?? '' }}">
                                                            </td>
                                                            <td><input type="number" name="room_types[{{ $index }}][price]" placeholder="0.00"
                                                                    step="0.01" style="width: 100%; padding: 5px;" value="{{ $roomType->price ?? '' }}"></td>
                                                            <td><input type="text" name="room_types[{{ $index }}][description]"
                                                                    placeholder="Description" style="width: 100%; padding: 5px;" value="{{ $roomType->description ?? '' }}">
                                                            </td>
                                                            <td>
                                                                <select multiple name="room_types[{{ $index }}][amenities][]" style="width: 100%; padding: 5px;">
                                                                    <option value="" disabled>Select Amenities</option>
                                                                    @foreach($amenities as $amenity)
                                                                        <option value="{{ $amenity->id }}" {{ in_array($amenity->id, $roomType->amenities ?? []) ? 'selected' : '' }}>{{ $amenity->name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </td>
                                                            <td>
                                                                <input type="file" name="room_types[{{ $index }}][images][]" accept="image/*"
    ...
    <script>
        ...
        let roomTypeCount = {{ isset($hotel) && $hotel->roomTypes ? count($hotel->roomTypes) : 1 }};
