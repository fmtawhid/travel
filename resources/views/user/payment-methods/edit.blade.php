	...
	<div class="db-2">
		<div class="db-2-com db-2-main">
			...
			<div class="db-2-main-com db2-form-pay db2-form-com">
				...
				<form action="{{ route('user.payment-methods.update', $paymentMethod) }}" method="POST" class="col s12">
					@csrf
					@method('PUT')
					...
					<div class="row">
						<div class="input-field col s12">
							...
							<input type="text" name="card_name" id="card_name" class="validate @error('card_name') is-invalid @enderror" 
								placeholder="Enter a name for this card" value="{{ old('card_name', $paymentMethod->card_name) }}" required>
							@error('card_name')
					...
					<div class="row">
						<div class="input-field col s12">
							...
							<input type="text" name="card_number" id="card_number" class="validate @error('card_number') is-invalid @enderror" 
								...
								value="{{ old('card_number', $paymentMethod->card_number) }}" required>
					...
					<div class="row">
						<div class="input-field col s12 m6">
							...
							<input type="text" name="expiry_date" id="expiry_date" class="validate @error('expiry_date') is-invalid @enderror" 
								placeholder="MM/YY" maxlength="5" value="{{ old('expiry_date', $paymentMethod->expiry_date) }}" required>
							@error('expiry_date')
								<span style="color: #dc3545; font-size: 12px;">{{ $message }}</span>
							@enderror
						</div>
						<div class="input-field col s12 m6">
							...
							<input type="text" name="cvv" id="cvv" class="validate @error('cvv') is-invalid @enderror" 
								...
								value="{{ old('cvv', $paymentMethod->cvv) }}" required>
							@error('cvv')
								<span style="color: #dc3545; font-size: 12px;">{{ $message }}</span>
							@enderror
						</div>
					...
					<div class="row">
						<div class="input-field col s12">
							...
							<input type="text" name="full_name" id="full_name" class="validate @error('full_name') is-invalid @enderror" 
								placeholder="Enter name as shown on card" value="{{ old('full_name', $paymentMethod->full_name) }}" required>
							@error('full_name')
								<span style="color: #dc3545; font-size: 12px;">{{ $message }}</span>
							@enderror
						</div>
					...
					<div class="row">
						<div class="input-field col s12">
							<label>
								<input type="checkbox" name="is_default" value="1" {{ old('is_default', $paymentMethod->is_default) ? 'checked' : '' }}>
					...
					<div class="row">
						<div class="col s12" style="display: flex; gap: 10px;">
							<input type="submit" value="UPDATE PAYMENT METHOD" class="waves-effect waves-light full-btn">
