                    ...
                    <tbody>
                        @forelse($bookings as $booking)
                            <tr>
                                ...
                                <td>{{ $booking->tour->title ?? $booking->package->name ?? 'N/A' }}</td>
                                <td>{{ $booking->tour->duration ?? $booking->package->duration ?? 'N/A' }}</td>
                                <td>{{ $booking->tour->start_date ? \Carbon\Carbon::parse($booking->tour->start_date)->format('d M Y') : 'N/A' }}</td>
                                <td>${{ $booking->tour->price ?? $booking->package->price ?? '0' }}</td>
